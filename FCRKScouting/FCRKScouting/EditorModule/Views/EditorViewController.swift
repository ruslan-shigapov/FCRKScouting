...
final class EditorViewController: UIViewController {
    ...
    @objc private func uploadPhotoButtonTapped() {
        ...
        let alertController = AlertFactory.getUploadPhotoActionSheet(
            hasPhotoAlreadyBeenUploaded: isPhotoUploaded
        ) { [weak self] in
            guard let self else { return }
            if UIImagePickerController.isSourceTypeAvailable(
                ...
                let alertText = Constants.Texts.Alerts.noAccessToPhotos
                let alertController = AlertFactory.getAlertController(
                    withTitle: alertText.title,
                    andMessage: alertText.message)
                present(alertController, animated: true)
            }
        } deletingCompletion: { [weak self] in
            guard let self else { return }
