    ...
    private func configureUI() {
        if viewModel.isFiltersActive {
            positionPickerView.selectRow(
                delegate.positionValue,
                inComponent: 0,
                animated: true)
            leaguePickerView.selectRow(
                delegate.leagueValue,
                inComponent: 0,
                animated: true)
            footSegmentedControl.selectedSegmentIndex = delegate.footValue
            ageTextFieldView.set(text: delegate.ageValue)
            if let toAgeValue = delegate.toAgeValue, !toAgeValue.isEmpty {
    ...
    @objc private func applyButtonTapped() {
        viewModel.checkRatioOf(
            ...
            DispatchQueue.main.asyncAfter(
                ...
                guard let self else { return }
                delegate.positionValue = positionPickerView.selectedRow(
                    inComponent: 0)
                delegate.leagueValue = leaguePickerView.selectedRow(inComponent: 0)
                delegate.footValue = footSegmentedControl.selectedSegmentIndex
                delegate.ageValue = ageTextFieldView.getInputText()
                delegate.toAgeValue = toAgeTextFieldView.getInputText()
                delegate.onExtraFiltersChanged?()
